...
tic;
%% Setup 
inputdata = 'ROMS_500m_395_wshelf.mat';
...
%         end
    ...
    ROMS_500m_unbinned.time = time;

    save([outputdatapath,'ROMS_500m_395_raw_15400pts_add40_noshelf150m.mat'],'-struct','ROMS_500m_unbinned','-v7.3');
    ...
    if sendmessage == true
        [h,m,s] = hms(datetime);
        if m<10
            send_text_message([outputdata, 'finished running at ',num2str(h),':0',num2str(m)])
        else
            send_text_message([outputdata, 'finished running at ',num2str(h),':',num2str(m)])
