...
parfor j = 1:nl-1
    ...
    ttmp{j}  = days(repmat(t(1,:),nn,1) - t(j+1:nl,:));
    
    dutmp{j} = repmat(u(1,:),nn,1) - u(j+1:nl,:);

    dvtmp{j} = repmat(v(1,:),nn,1) - v(j+1:nl,:);
    
    rtmp{j}  = m_idist(repmat(x(1,:),nn,1),...
    repmat(y(1,:),nn,1),x(j+1:nl,:),y(j+1:nl,:));
    
    dxtmp{j} = m_idist(repmat(x(1,:),nn,1),...
        repmat(y(1,:),nn,1),x(j+1:nl,:),repmat(y(1,:),nn,1));
    
    dxtmp{j}(repmat(x(1,:),nn,1) < x(j+1:nl,:)) = ...
        -dxtmp{j}(repmat(x(1,:),nn,1) < x(j+1:nl,:));
    
    dytmp{j} = m_idist(repmat(x(1,:),nn,1),...
        repmat(y(1,:),nn,1),repmat(x(1,:),nn,1),y(j+1:nl,:));
    
    dytmp{j}(repmat(y(1,:),nn,1) < y(j+1:nl,:)) = ...
        -dytmp{j}(repmat(y(1,:),nn,1) < y(j+1:nl,:));
